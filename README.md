...
같은 패널의 "코퍼스 문서"에서 .md/.txt/.pdf 파일을 끌어다 놓거나 선택해 올리고(PDF는 페이지별 섹션의 md로 변환), 삭제하고, 새 코퍼스를 만들 수 있습니다. 문서가 바뀌면 그 코퍼스를 쓰는 검색기가 다음 요청에서 자동으로 다시 인덱싱됩니다. 새 코퍼스 흐름: 코퍼스 만들기 → 문서 업로드 → 설정 조정 → "프리셋 저장".
삭제도 웹에서 합니다: 각 영역의 "현재 프리셋 삭제"(LLM/TTS/STT/프롬프트/RAG 옵션 YAML 삭제)와 "이 코퍼스 삭제"(문서 폴더와 임베딩 캐시 삭제). `default` 프롬프트와 각 종류의 마지막 옵션은 삭제할 수 없습니다. 코퍼스를 지우면 그 코퍼스를 가리키던 RAG 옵션 이름을 알려 주니 코퍼스를 바꾸거나 함께 삭제하세요.
