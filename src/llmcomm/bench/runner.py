...
from llmcomm.core.gpu import VRAMMonitor
    ...
    try:
        ...
        for p in prompts:
            for rep in range(repeats):
                msgs = [Message("system", system or p.get("system", DEFAULT_SYSTEM)), Message("user", p["user"])]
...
async def bench_e2e(llm_option: str, tts_option: str, prompt_set: str = "korean_chat", **gen) -> list[BenchRecord]:
    ...
    try:
        ...
        for p in prompts:
            msgs = [Message("system", p.get("system", DEFAULT_SYSTEM)), Message("user", p["user"])]
            text: list[str] = []
            audio_sec = 0.0
            metrics: dict = {}
            async with VRAMMonitor() as vram:
            ...
            metrics.update({"audio_sec": round(audio_sec, 2), "vram_used_mb": round(vram.peak_mb), "output_text": "".join(text)})
